...
void lerSeriesBinario(Serie *serie, int QuantidadeSeries, FILE *arquivo) {
    for (int i = 0; i < QuantidadeSeries; i++) {
        ...
        // Ler Nome até a vírgula
        fread(serie[i].Nome, sizeof(char), 101, arquivo);
        char *posicao_virgula = memchr(serie[i].Nome, ',', 101);
        if (posicao_virgula != NULL) {
            *posicao_virgula = '\0'; // Substituir a vírgula por \0
        }

        // Ler Genero até a vírgula
        fread(serie[i].Genero, sizeof(char), 41, arquivo);
        posicao_virgula = memchr(serie[i].Genero, ',', 41);
        if (posicao_virgula != NULL) {
            *posicao_virgula = '\0'; // Substituir a vírgula por \0
        }
        ...
        // Ler Plataforma até a vírgula
        fread(serie[i].Plataforma, sizeof(char), 41, arquivo);
        posicao_virgula = memchr(serie[i].Plataforma, ',', 41);
        if (posicao_virgula != NULL) {
            *posicao_virgula = '\0'; // Substituir a vírgula por \0
        }
...
void salvaDados(Serie *serie, int QuantidadeSeries)
{
    ...
    for (int i = 0; i < QuantidadeSeries; i++)
    {
        ...
        fwrite(&virgula, sizeof(char), 1, arquivobin);
